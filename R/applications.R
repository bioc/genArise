    ...
    ok.function <-  function(){
      ...
      if(!nchar(tclvalue(tkget(association.entry))) || !nchar(tclvalue(tkget(project.entry))) ||
        ...
        if(as.character(tclvalue(rbValue)) == "IFC"){
          temp <- temp[-1,] # aqui quitamos el encabezado
          row1 <- temp[,1]
          ...
          set.grid.properties(envir,name.project, as.integer(temp[length(temp[,1]),4]),
                              as.integer(temp[length(temp[,1]), 5]), as.integer(temp[length(temp[,1]), 2]),
                              as.integer(temp[length(temp[,1]), 3]))
        }
        else{
...
projects.select <- function(envir, nombre = "sets-analysis"){
  ...
  range  <- tclVar(1)
  updown <- tclVar(4)
  radio1 <- tkradiobutton(framerange, value=1, variable=range)
  radio2 <- tkradiobutton(framerange, value=2, variable=range)
  radio3 <- tkradiobutton(framerange, value=3, variable=range)
  ...
  radioup <- tkradiobutton(frameupdown, value=4, variable=updown)
  radiodown <- tkradiobutton(frameupdown, value=5, variable=updown)
  ...
  output <- tkentry(frameOverall,width="30", bg = "white", bg = "white", textvariable = tclVar(nombre))
  ...
  operacion <- function(){
    ...
    if (length(values) < 1)
      ...
      if(as.integer(tclvalue(updown)) == 4){
        up.down <- "up"
      }else{
        up.down <- "down"
      }
      post.analysis(values,
                    minimo, maximo, up.down = up.down, output = tclvalue(tkget(output)))
    }
  }
  ...
  ok.but <- tkbutton(button.frame, text ="OK", command = function(){
   ...
   tkconfigure(tt, cursor="arrow")
     tkmessageBox(title = "Post-Anlysis", icon="info", message=paste("The Post-Analysis has done.\n The results files are in this directory:", tclvalue(tkget(output))), type="ok")
    tkdestroy(tt)
   genArise()
  })
